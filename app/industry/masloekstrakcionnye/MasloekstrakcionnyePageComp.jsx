...
const advantages = [
    { title: 'Контроль масличности каждой партии', text: 'Лабораторный анализ масличности, влажности, сорной и масличной примеси перед отгрузкой. Протокол передаётся вместе с партией.' },
    { title: 'Стабильность между поставками', text: 'Показатели масличности и чистоты фиксируем в договоре. Каждая следующая партия — в пределах согласованного допуска.' },
    { title: 'Подбор под технические требования', text: 'Уточняем параметры переработки: целевой выход масла, требования к кислотному числу и влажности сырья.' },
...
const expertiseSteps = [
    { num: '01', title: 'Уточняем параметры переработки', text: 'Запрашиваем требования по масличности, влажности, кислотному числу и допустимому содержанию примесей под вашу линию.' },
...
const specifications = [
    {
        ...
        purpose: 'Производство подсолнечного масла и шрота',
        params: 'Масличность 48–52%, влажность ≤ 8%, сорная примесь ≤ 2%',
        delivery: 'Авто, EXW/FCA',
    },
    {
        product: 'Лён масличный',
        purpose: 'Производство льняного масла и жмыха',
        params: 'Масличность 44–46%, влажность ≤ 10%, сорная примесь ≤ 2%',
        delivery: 'Авто, EXW/FCA',
    },
    {
        product: 'Подсолнечник + лён (смешанная поставка)',
        purpose: 'Диверсификация сырья под производственный план',
        params: 'По согласованной спецификации',
...
const reviews = [
    {
        ...
        role: 'Технолог переработки',
        location: 'Ростов-на-Дону',
        date: 'Февраль 2026',
        rating: 5,
        text: 'Подсолнечник с масличностью 50%+ — стабильно на протяжении трёх поставок подряд. Протоколы анализов совпадают с нашими входными замерами. Именно такой партнёр нужен для планирования производства.',
    },
    {
        id: 2,
        name: 'Ирина Белова',
        ...
        date: 'Январь 2026',
        rating: 5,
        text: 'Брали лён масличный — влажность и сорная примесь строго в допуске. Документы оформлены без замечаний. Удобно, что всё включено в одну поставку с подсолнечником.',
    },
    {
        id: 3,
        name: 'Алексей Громов',
        ...
        date: 'Ноябрь 2025',
        rating: 5,
        text: 'Работаем второй сезон. Ценим предсказуемость: параметры фиксируют в договоре и выдерживают их в каждой машине. Для нас это принципиально важно.',
    },
];
...
function IndustryRelatedPosts() {
    const relatedPosts = posts
        .filter(p => p.category === 'Подсолнечник' || p.category === 'Лён')
        .slice(0, 3);
...
export default function MasloekstrakcionnyePageComp() {
    return (
        <div className="min-h-screen bg-[#263C28]">
            ...
            <div className="farm_container px-4 py-6">
                <nav className="flex items-center gap-2 text-sm text-[#EADCC1]/60 flex-wrap">
                    ...
                    <ChevronRight className="w-3 h-3" />
                    <span className="text-[#F7C35F]">Маслоэкстракционные заводы</span>
                </nav>
            </div>
                ...
                <div className="grid lg:grid-cols-2 gap-12 items-center">
                    <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
                        <span className="inline-block px-4 py-1.5 bg-[#F7C35F]/15 border border-[#F7C35F]/30 rounded-full text-[#F7C35F] text-sm font-semibold mb-6">
                            🛢️ Для маслоэкстракционных заводов
                        </span>
                        <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-[#F7C35F] leading-tight mb-6">
                            Подсолнечник масличный для производства растительного масла
                        </h1>
                        <p className="text-[#EADCC1] text-lg leading-relaxed mb-8">
                            КФХ Великанов поставляет подсолнечник с масличностью 48–52% и лён масличный 44–46% маслоэкстракционным заводам напрямую с хозяйства. Каждая партия — с лабораторным протоколом. Показатели масличности фиксируем в договоре.
                        </p>
                        <div className="flex flex-col sm:flex-row gap-4">
                            <Link href="#contact" className="inline-flex items-center justify-center bg-[#F7C35F] text-[#2c1810] font-bold px-8 py-4 rounded-[15px] hover:bg-[#e5b44f] transition-all shadow-lg hover:shadow-xl text-base">
                                Получить расчёт стоимости
                            </Link>
                            <Link href="/podsolnechnik" className="inline-flex items-center justify-center border border-[#F7C35F]/40 text-[#F7C35F] font-semibold px-8 py-4 rounded-[15px] hover:border-[#F7C35F] hover:bg-[#F7C35F]/10 transition-all text-base">
                                О нашем подсолнечнике
                            </Link>
                        </div>
                    </motion.div>
                    <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.15 }} className="grid grid-cols-2 gap-4">
                        {[
                            ...
                            { value: '44–46%', label: 'масличность', sub: 'лён масличный' },
                            { value: '≤ 8%', label: 'влажность', sub: 'по ГОСТ' },
            ...
            <section className="farm_container px-4 py-16 md:py-20">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-12">
                    <h2 className="text-2xl md:text-3xl font-bold text-[#F7C35F] mb-4">Почему маслоэкстракционные заводы выбирают нас?</h2>
                    <p className="text-[#EADCC1]/70 text-lg max-w-2xl">Для МЭЗа критичны масличность сырья и предсказуемость каждой следующей партии. Именно на это мы делаем акцент.</p>
                </motion.div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <section className="farm_container px-4 py-16 md:py-20">
                <div className="bg-gradient-to-br from-[#2c1810]/40 to-[#1a0e08]/40 border border-[#F7C35F]/20 rounded-3xl p-8 md:p-12">
                    <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
                        <h2 className="text-2xl md:text-3xl font-bold text-[#F7C35F] mb-4">Как мы подбираем сырьё под задачи вашего производства?</h2>
                        <p className="text-[#EADCC1]/70 text-lg max-w-2xl">Масличное сырьё для МЭЗа — это не просто культура с нужным процентом. Мы учитываем параметры линии экстракции, требования к влажности и допустимую сорность.</p>
                    </motion.div>
            ...
            <section className="farm_container px-4 py-16 md:py-20">
                ...
                <PriceCalculator productName="подсолнечника" basePrice={28000} minVolume={20} />
            </section>
            ...
            <section className="farm_container px-4 py-16 md:py-20">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
                    ...
                    <p className="text-[#EADCC1]/70">Статьи о масличных культурах, показателях сырья и агрономических нюансах.</p>
                </motion.div>
                <IndustryRelatedPosts />
                <div className="mt-8 text-center">
                    <Link href="/blog" className="inline-flex items-center gap-2 text-[#F7C35F] font-semibold hover:text-[#e5b44f] transition-colors">
                        Все материалы в блоге <ChevronRight className="w-4 h-4" />
                    </Link>
                </div>
            </section>
            ...
            <section className="farm_container px-4 py-16 md:py-20">
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-10">
                    <h2 className="text-2xl md:text-3xl font-bold text-[#F7C35F] mb-4">Нам доверяют маслоэкстракционные заводы</h2>
                    <p className="text-[#EADCC1]/70">Акцент — на стабильность масличности и предсказуемость каждой следующей поставки.</p>
                </motion.div>
            ...
            <section className="farm_container px-4 pb-24">
                <div className="bg-gradient-to-r from-[#F7C35F]/15 via-[#F7C35F]/10 to-[#F7C35F]/5 border border-[#F7C35F]/30 rounded-3xl p-10 md:p-14 text-center">
                    <h2 className="text-2xl md:text-3xl font-bold text-[#F7C35F] mb-4">Готовы обсудить поставку под ваши технические требования?</h2>
                    <p className="text-[#EADCC1]/70 mb-8 max-w-xl mx-auto leading-relaxed">Оставьте заявку — уточним культуру, масличность, объём и сроки. Ответим в рабочее время в день обращения.</p>
